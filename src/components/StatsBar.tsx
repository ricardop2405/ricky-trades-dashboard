...
export const StatsBar = ({ bundles, trades }: StatsBarProps) => {
  ...
  const stats = [
    { label: "Profit", value: `$${totalProfit.toFixed(2)}`, icon: TrendingUp, color: "text-success", glowClass: "stat-glow" },
    ...
    { label: "Latency", value: `${avgLatency.toFixed(0)}ms`, icon: Clock, color: "text-warning" },
    { label: "Streak", value: winStreak.toString(), icon: Flame, color: "text-success" },
    { label: "Best", value: `$${largestProfit.toFixed(2)}`, icon: Trophy, color: "text-accent" },
    ...
    <div className="grid grid-cols-7 gap-2">
      {stats.map((stat, i) => (
        <motion.div
          ...
          className={`glass-elevated rounded-xl px-3 py-2.5 ${stat.glowClass || ""}`}
        >
          ...
          <span className={`text-lg font-mono font-bold ${stat.color} tracking-tight`}>
            {stat.value}
          </span>
        </motion.div>
      ))}
